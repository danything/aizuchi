...
public class BotTests
{
    ...
    private static readonly BotOptions Options = new("sys", 50, TimeSpan.Zero, null, 8000, 0);
    ...
    public async Task 履歴が空なら何もしない()
    {
        ...
        await new Bot(new FakeProvider(["x"]), Options, null, [], NullLogger.Instance).HandleAsync(new IncomingMessage("c", "C1", "hi", conv), TestContext.Current!.Execution.CancellationToken);
    ...
    public async Task 途中切れと拒絶は文言を添える()
    {
        ...
        await new Bot(new FakeProvider(["長い"], StopKind.Truncated), Options, null, [], NullLogger.Instance).HandleAsync(new IncomingMessage("c", "C1", "hi", truncated), TestContext.Current!.Execution.CancellationToken);
        await Assert.That(truncated.Final).StartsWith("長い\n\n_(出力上限");

        var refused = new FakeConversation(History);
        await new Bot(new FakeProvider(["途中"], StopKind.Refused), Options, null, [], NullLogger.Instance).HandleAsync(new IncomingMessage("c", "C1", "hi", refused), TestContext.Current!.Execution.CancellationToken);
